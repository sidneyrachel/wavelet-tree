...
class Node(object):
    ...
    def get_rank_bit(self, position=None, bit=None):
        if position is None or bit is None:
            print('Please give correct parameters')
            return -1
        rs_position = int(position/(BLOCKS_NUM*BITS_NUM))  # Calculate rs and rb position
        rb_position = int(position/BITS_NUM)
    ...
    def __create_RRR(self):  # Create the RRR Node that make the rank very fast
        ...
        # Calculate how many True(1) have the Node, to calculate the rs and rb.
        # rs is for the super block and rb is for the block
